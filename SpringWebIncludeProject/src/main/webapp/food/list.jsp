...
<body>
	...
	<div style="height:10px"></div>
			...
			<div class="text-center">
				<ul class="pagination">
					...
					<c:forEach var="i" begin="${startPage }" end="${endPage }">
						<li ${i==curpage?"calss=active":"" }><a href="../food/list.do?page=${i }">${i }</a></li>
					</c:forEach>
		...
		<h3>방문 맛집</h3>
		<a href="../food/food_cookie_all.do" class="btn btn-xs btn-primary">더보기</a>
		<hr>
		<div class="row">
			<c:if test="${size==0 }">
			...
			<c:if test="${size>0 }">
				<c:forEach var="vo" items="${fcList }" varStatus="s">
					<c:if test="${s.index<9 }">
					<a href="../food/detail.do?fno=${vo.fno }">
						<img src="http://menupan.com${vo.poster }" style="width:100px; height:100px;" title="${vo.address	}">
					</a>
					</c:if>
				</c:forEach>
			</c:if>
